...
function isTrainRemReferenceOnlyIndex(depot = "west", label = "9am", rowIndex = -1, activeTimetable = null) {
  return isTrainRemCombinedReferencePreset(depot, label)
    && rowIndex >= 0
    && rowIndex < getTrainRemReferenceTids(label, activeTimetable).length;
}
...
function mergeTrainRemCombinedMorningReferenceState(state = {}, activeTimetable = null) {
  ...
  const referenceRowCount = getTrainRemReferenceTids(westPreset, activeTimetable).length;

  const mergedWestRows = westRows.map((row, index) => {
    if (index >= referenceRowCount || normalizeTrainId(row?.trainId || "")) return row;
  ...
  return (
    <section className="theme-train-rem-panel w-[314px] flex-shrink-0 rounded-xl border border-[#2b4f6b] bg-[#0b1f33] p-2 shadow-md">
      ...
      <div className="space-y-1.5">
        ...
        {!isTrainRemLegacyCombinedReferencePreset("west", trainRemState.selectedPreset?.west || "9am") && renderDepotTable("east", "East Depot", "")}
      </div>
    </section>
  );
}
...
function getTrainRemRemovalEntries(trainRemState = {}, depot = "west", maintenanceMap = {}, activeTimetable = null, stablingData = {}) {
  ...
  if (useCombinedReference) {
    ...
    westRows.forEach((row, index) => {
      ...
      // The four trailing rows on the combined West table are explicit
    ...
    // 12am / Fri / Sat / PH keep a separate 14-row East table for
    // additional East removals. Scheduled East trains are already represented
    // in the combined West reference list, so matching schedule TIDs are skipped
    // here to prevent duplicate PDF/output rows.
    if (safeDepot === "east" && TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(westSelectedPreset)) {
      const eastSelectedPreset = trainRemState?.selectedPreset?.east || westSelectedPreset;
      const eastSourceRows = eastSelectedPreset === westSelectedPreset
        ? trainRemState?.rows?.east
      ...
      eastRows.forEach((row, index) => {
        ...
        candidates.push({
          ...
          tid,
          time: cleanRemovalTime(row?.timing),
          originalIndex: referenceRowCount + index,
        });
      });
    }
  } else {
