...
function buildTrainRemRowsFromPresetConfig(
  ...
  if (isTrainRemCombinedReferencePreset(depot, label)) {
    return buildTrainRemCombinedWestRows(existingRows, label, activeTimetable);
  }

  if (depot === "east" && TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)) {
    return buildTrainRemAdditionalEastRows(existingRows, label, activeTimetable, {
...
const TRAIN_REM_ROW_COUNTS = { west: 44, east: 14 };
const TRAIN_REM_WEST_DEFAULT_VISIBLE_ROW_COUNT = 32;
const TRAIN_REM_WEST_COMPACT_PRESET_LABELS = new Set(["12am", "Fri", "Sat", "PH"]);
const TRAIN_REM_WEST_COMPACT_EMPTY_ROW_COUNT = 4;
const TRAIN_REM_LEGACY_COMBINED_PRESET_LABELS = new Set(["9am", "7pm"]);
const TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS = new Set(["12am", "Fri", "Sat", "PH"]);
const TRAIN_REM_ADDITIONAL_WEST_ROW_COUNT = 4;
const TRAIN_REM_ADDITIONAL_EAST_ROW_COUNTS = { "12am": 3, "Fri": 3, "Sat": 3, "PH": 3 };
...
function isTrainRemLegacyCombinedReferencePreset(depot = "west", label = "9am") {
  return depot === "west" && TRAIN_REM_LEGACY_COMBINED_PRESET_LABELS.has(label);
}

function isTrainRemExtendedCombinedReferencePreset(depot = "west", label = "9am") {
  return depot === "west" && TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label);
}
...
function getTrainRemReferenceTids(label = "9am", activeTimetable = null) {
  if (label === "7pm") return TRAIN_REM_WEST_7PM_REFERENCE_TIDS.map((tid) => String(tid));
  if (label === "9am") return TRAIN_REM_WEST_9AM_REFERENCE_TIDS.map((tid) => String(tid));
  if (!TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)) return [];

  const combinedTids = [
    ...(getTrainRemPresetConfig("west", label, activeTimetable)?.tids || []),
    ...(getTrainRemPresetConfig("east", label, activeTimetable)?.tids || []),
  ];
  const seen = new Set();

  return combinedTids
    .map((tid) => normalizeTrainRemTidValue(tid))
    .filter((tid) => tid && !seen.has(tid) && seen.add(tid));
}

function getTrainRemCombinedExtendedLayout(label = "9am", activeTimetable = null) {
  const westTids = (getTrainRemPresetConfig("west", label, activeTimetable)?.tids || [])
    .map((tid) => normalizeTrainRemTidValue(tid))
    .filter(Boolean);
  const eastTids = (getTrainRemPresetConfig("east", label, activeTimetable)?.tids || [])
    .map((tid) => normalizeTrainRemTidValue(tid))
    .filter(Boolean);
  const reserveCount = getTrainRemAdditionalWestRowCount(label);
  const eastReserveCount = getTrainRemAdditionalEastRowCount(label);
  const eastStartIndex = westTids.length + reserveCount;
  const eastReserveStartIndex = eastStartIndex + eastTids.length;
  ...
  return {
    ...
    reserveCount,
    eastReserveCount,
    westCount: westTids.length,
    eastCount: eastTids.length,
    reserveStartIndex: westTids.length,
    eastStartIndex,
    eastReserveStartIndex,
    totalCount: eastReserveStartIndex + eastReserveCount,
  };
}

function getTrainRemAdditionalWestRowCount(label = "9am") {
  return TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)
    ? TRAIN_REM_ADDITIONAL_WEST_ROW_COUNT
    : 0;
}

function getTrainRemAdditionalEastRowCount(label = "9am") {
  return TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)
    ? TRAIN_REM_ADDITIONAL_EAST_ROW_COUNTS[label] || 0
    : 0;
}

function isTrainRemCombinedWestReserveRowIndex(label = "9am", rowIndex = -1, activeTimetable = null) {
  if (!TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label) || rowIndex < 0) return false;
  const layout = getTrainRemCombinedExtendedLayout(label, activeTimetable);
  return rowIndex >= layout.reserveStartIndex && rowIndex < layout.eastStartIndex;
}

function isTrainRemCombinedEastExtraRowIndex(depot = "west", label = "9am", rowIndex = -1, activeTimetable = null) {
  if (depot !== "west" || !TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label) || rowIndex < 0) return false;
  const layout = getTrainRemCombinedExtendedLayout(label, activeTimetable);
...
function isTrainRemReferenceOnlyIndex(depot = "west", label = "9am", rowIndex = -1, activeTimetable = null) {
  if (!isTrainRemCombinedReferencePreset(depot, label) || rowIndex < 0) return false;

  if (TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label) && depot === "west") {
    const layout = getTrainRemCombinedExtendedLayout(label, activeTimetable);
    return rowIndex < layout.westCount
      || (rowIndex >= layout.eastStartIndex && rowIndex < layout.eastReserveStartIndex);
  }

  return rowIndex < getTrainRemReferenceTids(label, activeTimetable).length;
}

function getTrainRemPresetRowTids(depot = "west", label = "9am", tids = [], activeTimetable = null) {
  ...
  if (TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label) && depot === "west") {
    ...
    return [
      ...layout.westTids,
      ...Array.from({ length: layout.reserveCount }, () => ""),
      ...layout.eastTids,
      ...Array.from({ length: layout.eastReserveCount }, () => ""),
    ];
  }

  return getTrainRemReferenceTids(label, activeTimetable).map((tid) => String(tid));
}
...
function hasTrainRemRowContent(row = {}) {
  return [row?.trainId, row?.tid, row?.timing, row?.remark]
    .some((value) => String(value || "").trim());
}

function buildTrainRemCombinedWestRows(existingRows = [], label = "9am", activeTimetable = null) {
  const referenceTids = getTrainRemReferenceTids(label, activeTimetable);
  const referenceTidSet = new Set(referenceTids.map((tid) => normalizeTrainRemTidValue(tid)));
  const rowsByTid = indexTrainRemRowsByTid(existingRows);
  const normalizedSourceRows = normalizeTrainRemRows(existingRows, "west");
  ...
  if (TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)) {
    ...
    const eastReferenceRows = layout.eastTids.map((referenceTid) => {
      const tid = normalizeTrainRemTidValue(referenceTid);
      const matchedRow = rowsByTid.get(tid) || {};
    ...
    return [...westReferenceRows, ...additionalRows, ...eastReferenceRows, ...eastAdditionalRows];
  }

  const referenceRows = referenceTids.map((referenceTid) => {
    const tid = normalizeTrainRemTidValue(referenceTid);
    const matchedRow = rowsByTid.get(tid) || {};
  ...
  const additionalRowCount = getTrainRemAdditionalWestRowCount(label);
  if (!additionalRowCount) return referenceRows;
  ...
  while (additionalRows.length < additionalRowCount) {
    additionalRows.push({ trainId: "", tid: "", timing: "", remark: "" });
  }

  return [...referenceRows, ...additionalRows];
}

function buildTrainRemAdditionalEastRows(existingRows = [], label = "9am", activeTimetable = null, { migrateScheduledPlaceholders = false } = {}) {
  const rows = normalizeTrainRemRows(existingRows, "east");
  if (!TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)) return rows;
  if (!migrateScheduledPlaceholders) return rows;

  const scheduledTidSet = new Set(
    (getTrainRemPresetConfig("east", label, activeTimetable)?.tids || [])
      .map((tid) => normalizeTrainRemTidValue(tid))
      .filter(Boolean)
  );
...
function mergeTrainRemCombinedMorningReferenceState(state = {}, activeTimetable = null) {
  ...
  const normalizedEastPresetRows = normalizeTrainRemRows(eastPresetRows, "east");
  const eastRowsByTid = indexTrainRemRowsByTid(normalizedEastPresetRows);

  let mergedWestRows = westRows.map((row, index) => {
    if (!isTrainRemReferenceOnlyIndex("west", westPreset, index, activeTimetable) || normalizeTrainId(row?.trainId || "")) return row;
...
function collectTrainRemTrainIdsForDepotCopy(
  ...
  if (useCombinedReference) {
    ...
    westRows.forEach((row, index) => {
      const tid = normalizeTrainRemTidValue(row?.tid || "");
      const isReferenceRow = isTrainRemReferenceOnlyIndex("west", westSelectedPreset, index, activeTimetable);
      ...
      if (safeDepot === "east" && isEastExtraRow) {
        rowsToScan.push(row);
        return;
      }

      if (safeDepot === "west" && !isEastExtraRow) rowsToScan.push(row);
    });

    if (safeDepot === "east" && TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(westSelectedPreset)) {
      const eastSelectedPreset = trainRemState?.selectedPreset?.east || westSelectedPreset;
      const eastSourceRows = eastSelectedPreset === westSelectedPreset
        ? trainRemState?.rows?.east
        : trainRemState?.presetRows?.east?.[westSelectedPreset];
      const eastRows = normalizeTrainRemRowsForPreset(
        eastSourceRows,
        "east",
        westSelectedPreset,
        activeTimetable
      ...
      eastRows.forEach((row) => {
        const tid = normalizeTrainRemTidValue(row?.tid || "");
        if (tid && getTrainRemScheduleMatch(activeTimetable, "east", westSelectedPreset, tid)) return;
        rowsToScan.push(row);
      });
    }
  } else {
    normalizeTrainRemRowsForPreset(
      trainRemState?.rows?.[safeDepot],
      safeDepot,
      selectedPreset,
      activeTimetable
    ).forEach((row, index) => {
      if (isTrainRemReferenceOnlyIndex(safeDepot, selectedPreset, index, activeTimetable)) return;
      rowsToScan.push(row);
    });
  }
...
function buildTrainRemRowsFromPreset(depot, label, existingRows = []) {
  ...
  if (isTrainRemCombinedReferencePreset(depot, label)) {
    return buildTrainRemCombinedWestRows(existingRows, label);
  }

  if (depot === "east" && TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(label)) {
    return buildTrainRemAdditionalEastRows(existingRows, label);
  }
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div className="flex min-w-0 flex-wrap items-center justify-start gap-3">
        ...
        <h2 className="theme-insertion-title truncate text-[21px] font-black uppercase leading-none tracking-wide text-white">
          {title}
        </h2>
...
function InsertionCell({ block, bi, road, labelSide, isLast, isFirstBlock, isLastBlock, maintenanceMap, insertionLog, onInsertionTick, onInsertionTimeUpdate, onInsertionRemarkUpdate, onInsertionTaNameUpdate, onSweepUpdate, tidInput, onTidChange, onTidKeyDown, onTidFocus, tidInputRef, hideElapsedTid, getTidScheduledTime, getTidAssistRemark, getTidAssistRemarkStyle, isWeekdayActive = false, duplicateTidKeys = null, stablingEditable = false, onEditableTrainIdChange, rowCardMinHeight = 98, rowMaintenanceSlotHeight = 0, tidDropRequest = null, onTidDropApplied, isTidDragActive = false, isTidDropHovered = false }) {
  ...
  const isDuplicateInsertedTid = Boolean(
    isWeekdayActive &&
    insertedTid &&
    duplicateTidKeys?.has?.(String(insertedTid))
  );
        ...
        <table className="theme-insertion-table border-separate border-spacing-0 table-fixed text-xs" style={{ minWidth: 880, maxWidth: 880, width: 880 }}>
          <thead>
            <tr>
              ...
              {labelSide === "right" && <th className="theme-insertion-table-header w-[68px]" style={{ background: INSERTION_PANEL_COLORS.header, border: "none", borderBottom: `1px solid ${INSERTION_PANEL_COLORS.gridLine}` }} />}
            </tr>
          </thead>
          <tbody>
            {roads.map((road, ri) => {
...
function TrainRemPanel({ maintenanceMap = {}, onTrainRemStateChange, eastStablingData = {}, requests = [], westData = {}, eastData = {}, activeTimetable = null, activeTimetableType = "weekday" }) {
  ...
  const getTrainRemDuplicateCounts = () => {
    ...
    scanDepots.forEach((scanDepot) => {
      ...
      const scanRows = normalizeTrainRemRowsForPreset(
        trainRemState.rows?.[scanDepot],
        scanDepot,
        scanPreset,
        activeTimetable
      );
  ...
  const applyPreset = (depot, label) => {
    updateTrainRemState((prev) => {
      ...
      return mergeTrainRemCombinedMorningReferenceState(nextState, activeTimetable);
    });
  };
  ...
  const updateTrainRemCell = (depot, rowIndex, field, value) => {
    updateTrainRemState((prev) => {
      ...
      const rows = normalizeTrainRemRowsForPreset(prev.rows?.[depot], depot, presetLabel, activeTimetable);
      const referenceOnly = isTrainRemReferenceOnlyIndex(depot, presetLabel, rowIndex, activeTimetable);
  ...
  const renderDepotTable = (depot, title, subtitle) => {
    ...
    const displayRowEntries = activeSortMode === "color"
      ? [...rowEntries].sort((a, b) => {
          const aGroup = getRemovalColorSortGroup(a.row, a.sourceIndex);
          const bGroup = getRemovalColorSortGroup(b.row, b.sourceIndex);
          ...
          <table className="theme-train-rem-table w-full border-separate border-spacing-0 table-fixed text-[12px]">
            ...
            <tbody>
              {displayRowEntries.map(({ row, sourceIndex: index }, displayIndex) => {
                ...
                const combinedExtendedLayout = TRAIN_REM_EXTENDED_COMBINED_PRESET_LABELS.has(selectedPreset)
                  ? getTrainRemCombinedExtendedLayout(selectedPreset, activeTimetable)
                  : null;
                const isCombinedWestReserveRow = isTrainRemCombinedWestReserveRowIndex(
                  selectedPreset,
                  index,
                  activeTimetable
                );
                const isCombinedEastExtraRow = isTrainRemCombinedEastExtraRowIndex(
                  depot,
                  selectedPreset,
                  index,
                  activeTimetable
                );
                const isEmptyCombinedManualReserveRow = (isCombinedWestReserveRow || isCombinedEastExtraRow)
                  && !String(row.trainId || "").trim()
                  && !String(row.tid || "").trim()
                  && !String(displayTimingValue || "").trim()
                ...
                const isExtendedEastPresetRow = Boolean(isEastReferenceRemoval || isCombinedEastExtraRow)
                ...
                const isOtherPresetThemedRow = selectedPreset !== "9am"
                  ...
                  && (hasTid || isCombinedWestReserveRow || isCombinedEastExtraRow)
                ...
                const rowCardVisual = getTrainRemRowCardVisual(
                  ...
                  "",
                  {
                    hasContent: hasRowContent || isEmptyCombinedManualReserveRow,
                ...
                const tidTextColor = isDuplicateTid
                  ? "#fecaca"
                  : isNineAmReferenceTid || isNineAmSpecialTid || isNineAmOtherTid || isOtherPresetThemedRow
                    ? "#ffffff"
                    : cleanTid.length === 3
                      ? "#dbeafe"
                      : hasTid
...
function InsertionTabContent({ westSection, eastSection, maintenanceMap, insertionLog, onClearInsertionDepot, getTidScheduledTime, getTidAssistRemark, getTidAssistRemarkStyle, activeTimetable, activeTimetableType, insertionLiveStatusText, insertionLiveStatusClass, insertionLiveDebug, eastInsertionTimeOffsetMinutes = 0, onEastInsertionTimeOffsetChange }) {
  ...
  const insertionAssistDuplicateTidKeySet = useMemo(
    ...
    [insertionAssistTidUsage.duplicateTidKeys]
  );
...
function isTrainMovementExcelLiveEntityReady(entity = getTrainMovementExcelLiveEntity()) {
  return Boolean(entity && typeof entity.list === "function" && typeof entity.create === "function" && typeof entity.update === "function");
}
...
function normalizeTrainMovementExcelLogEntry(entry = {}) {
  return {
    id: entry.id || `excel-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    depot: entry.depot === "east" ? "east" : "west",
    operation: ["swapping", "insertion", "removal"].includes(entry.operation) ? entry.operation : "swapping",
    time: normalizeMovementCustomTimeInput(entry.time),
...
function loadTrainMovementExcelLogRows() {
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.map(normalizeTrainMovementExcelLogEntry).filter((entry) => entry.text).slice(0, 120);
    }
  } catch {}
  return [];
}

function saveTrainMovementExcelLogRows(rows = []) {
...
function normalizeTrainMovementExcelLiveState(source = {}) {
  const normalizedRows = Array.isArray(source.rows)
    ? source.rows.map(normalizeTrainMovementExcelRow).slice(0, 60)
    : [];

  const normalizedLogRows = Array.isArray(source.logRows)
    ? source.logRows.map(normalizeTrainMovementExcelLogEntry).filter((entry) => entry.text).slice(0, 120)
    : [];
...
function buildTrainMovementExcelLivePayload(state = {}) {
  const normalized = normalizeTrainMovementExcelLiveState(state);
  return {
...
function sortTrainMovementExcelLogRows(rows = []) {
  return [...(Array.isArray(rows) ? rows : [])].sort((a, b) => {
    const aTime = excelTimeToMinutes(a?.time || "");
    const bTime = excelTimeToMinutes(b?.time || "");
...
function buildMovementExcelRemarkSuffix(value = "") {
  const clean = String(value || "").trim();
  if (!clean) return "";
...
function buildMovementExcelLogLine(row = {}) {
  ...
  if (operation === "removal") {
    const remarkSuffix = buildMovementExcelRemarkSuffix(String(row.reason || notes || "").trim());
    return `${time} hrs – ${train}${tidPart} removed from mainline to ${depotLabel}.${remarkSuffix}`;
  }
...
function TrainMovementExcelSheet() {
  ...
  const trainMovementExcelRecordIdRef = useRef(null);
  const trainMovementExcelAutoSaveTimerRef = useRef(null);
  const trainMovementExcelSavingRef = useRef(false);
  ...
  const scheduleTrainMovementExcelLiveSave = useCallback((state) => {
    ...
    if (trainMovementExcelAutoSaveTimerRef.current) {
      clearTimeout(trainMovementExcelAutoSaveTimerRef.current);
    }
  ...
  useEffect(() => {
    refreshTrainMovementExcelLiveFromDb({ showStatus: true });
  }, [refreshTrainMovementExcelLiveFromDb]);

  useEffect(() => {
    if (!liveLoaded || !liveDbReady) return;

    const interval = setInterval(() => {
      refreshTrainMovementExcelLiveFromDb({ showStatus: true });
    }, TRAIN_MOVEMENT_EXCEL_LIVE_SYNC_INTERVAL_MS);

    return () => clearInterval(interval);
  }, [liveLoaded, liveDbReady, refreshTrainMovementExcelLiveFromDb]);

  useEffect(() => {
    const payload = { rows, logRows };
  ...
  useEffect(() => () => {
    ...
    if (trainMovementExcelAutoSaveTimerRef.current) clearTimeout(trainMovementExcelAutoSaveTimerRef.current);
  }, []);
  ...
  const clearRows = () => {
    ...
    markTrainMovementExcelLocalEdit();
    setRows(getDefaultTrainMovementExcelRows());
    resetClearConfirm();
  ...
  const addRowToMovementLog = (row) => {
    ...
    const now = new Date();
    const entry = normalizeTrainMovementExcelLogEntry({
      id: `movement-excel-log-${now.getTime()}-${Math.random().toString(36).slice(2, 7)}`,
      depot: row.depot === "east" ? "east" : "west",
      operation: row.operation || "swapping",
      time: normalizeMovementCustomTimeInput(row.time),
  ...
  const copyExcelLogRows = async (depot = "") => {
    const lines = sortedLogRows
      .filter((entry) => !depot || entry.depot === depot)
...
function getWestRemovalRowsMap(trainRemState = {}, activeTimetable = null) {
  ...
  if (isTrainRemCombinedReferencePreset("west", selectedPreset)) {
    westRows.forEach((row, index) => {
      ...
      const tid = normalizeTrainRemTidValue(row.tid);
      const isReferenceRow = isTrainRemReferenceOnlyIndex("west", selectedPreset, index, activeTimetable);
      const isEastExtraRow = isTrainRemCombinedEastExtraRowIndex("west", selectedPreset, index, activeTimetable);
      const scheduleMatch = isReferenceRow
        ? getTrainRemScheduleMatch(activeTimetable, "west", selectedPreset, tid)
        : null;

      if (isReferenceRow && !scheduleMatch) return;
      if (isEastExtraRow) return;

      map.set(key, {
        tid,
        timing: scheduleMatch?.timing || (row.timing || "").toString().trim(),
        remark: (row.remark || "").toString().trim(),
        rowIndex: index,
        selectedPreset,
        isTrainRemReferenceOnly: isReferenceRow,
        // Legacy property name retained for requested-train compatibility.
        isWest9amReferenceOnly: isReferenceRow,
        isWest9amRealRemoval: selectedPreset === "9am" && Boolean(scheduleMatch),
        isWest7pmRealRemoval: selectedPreset === "7pm" && Boolean(scheduleMatch),
      });
    });
...
function getTrainRemRemovalEntries(trainRemState = {}, depot = "west", maintenanceMap = {}, activeTimetable = null, stablingData = {}) {
  ...
  if (useCombinedReference) {
    ...
    westRows.forEach((row, index) => {
      ...
      if (safeDepot === "east" && isEastExtraRow) {
        candidates.push({
          row,
          tid,
          time: cleanRemovalTime(row?.timing),
          originalIndex: index,
        });
        return;
      }

      // The reserve rows inserted after the West scheduled TIDs are explicit
      // additional West removals. The 12am East extra rows are routed only to East.
      if (safeDepot === "west" && !isEastExtraRow) {
        candidates.push({
          row,
          tid,
          time: cleanRemovalTime(row?.timing),
    ...
    // 12am / Fri / Sat / PH follow the combined single-panel layout. East Depot
    // scheduled rows and the 12am East extra rows are represented inside the
    // combined West list, so no hidden East-panel rows are appended.
  } else {
    const sourceRows = normalizeTrainRemRowsForPreset(
      trainRemState?.rows?.[safeDepot],
      safeDepot,
      selectedPreset,
      activeTimetable
  ...
  return candidates
    .map(({ row, tid, time, originalIndex }) => {
      ...
      return {
        ...
        sortMinutes: getRemovalTimeMinutes(adjustedTime),
        originalIndex,
      };
    })
    .filter(Boolean)
...
function buildCombinedRemovalPdfPage(westLog = {}, eastLog = {}, options = {}) {
  ...
  const formatRequestedSummaryTrainList = (values = []) => {
    const trains = [...new Set((values || []).filter(Boolean))];
    if (trains.length === 0) return "-";
    if (trains.length === 1) return trains[0];
    if (trains.length === 2) return `${trains[0]} and ${trains[1]}`;
    return `${trains.slice(0, -1).join(", ")} and ${trains[trains.length - 1]}`;
  };

  const requestedSummaryGroups = (() => {
    const groups = new Map();

    actionOverviewRows.forEach((row) => {
      if (!row || row.isSeparator) return;

      const trainNumber = formatRequestedTrainNumber(row?.trainsetNumber || row?.key);
      const trainLabel = trainNumber ? `T${String(trainNumber).padStart(2, "0")}` : "";
      if (!trainLabel) return;

      splitRequestedActionRemarks(row?.requestType || "")
        .filter((remark) => !isWashRequestedActionRemark(remark))
        .forEach((remark) => {
          const identity = normalizeRequestIdentity(remark);
          if (!identity) return;

          if (!groups.has(identity)) {
            const pillStyle = getRequestedRemarkPillItems(remark)?.[0] || {};
            groups.set(identity, {
              key: identity,
              label: remark,
          ...
          if (!group.trains.includes(trainLabel)) group.trains.push(trainLabel);
        });
    });

    const getSummaryPriority = (label = "") => {
      const text = normalizeRequestIdentity(label);
      if (text.includes("DEEP CLEAN")) return 10;
      if (text.includes("INBOUND")) return 20;
      if (/\bPM\b/.test(text)) return 30;
      return 40;
    };

    return [...groups.values()]
      .sort((a, b) => (getSummaryPriority(a.label) - getSummaryPriority(b.label)) || (a.firstIndex - b.firstIndex))
      .map((group) => ({
        ...group,
        trainText: formatRequestedSummaryTrainList(group.trains),
      }));
  })();
  ...
  const stackedActionRowHeight = Math.max(
    8.4,
    Math.min(14.2, (stackedActionAvailableHeight - headerHeight) / actionOverviewRowCount)
  );
  ...
  const drawRequestedTrainSummary = (x, summaryTop, tableWidth) => {
    if (!requestedSummaryRows.length) return;

    let currentTop = summaryTop;
    requestedSummaryRows.forEach((group) => {
      ...
      trainLines.forEach((lineText, lineIndex) => {
        ops += pdfText(lineText, trainTextX, firstBaseline - lineIndex * lineHeight, {
          size: trainFontSize,
          color: "#000000",
          font: "F2",
        });
  ...
  if (stackMorningDepots) {
    drawRemovalColumn(westLog, marginX, "west", {
      ...
      fontReferenceRowHeight: Math.min(stackedActionRowHeight, stackedRemovalRowHeight),
      contentFontBoost: stackedRemovalRowHeight < 9 ? 0 : 2,
    });
