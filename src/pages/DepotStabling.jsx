...
function PSTCell({ block, bi, road, labelSide, isLast, isFirstBlock, isLastBlock, maintenanceMap, pstState, prepState, onPSTTick, onPrepTick, taName, onTaNameChange }) {
              ...
              return (
                <tr key={road}>
                  ...
                  {blockIndices.map((bi, i) => (
                    <PSTCell key={bi} block={data[road]?.[bi]} bi={bi} road={road} labelSide={labelSide} isLast={ri === roads.length - 1} isFirstBlock={i === 0} isLastBlock={i === blockIndices.length - 1} maintenanceMap={maintenanceMap} pstState={pstState} prepState={prepState} onPSTTick={onPSTTick} onPrepTick={onPrepTick} taName={taNameState[`${road}-${bi}`] || ""} onTaNameChange={onTaNameChange} />
                  ))}
...
function TrainMovementContent() {
  ...
  const renderTimingPicker = (operation) => {
    ...
    return (
      <div>
        <span className="mb-1.5 block text-[14px] font-medium uppercase tracking-[0.12em] text-[#58a6ff]">Timing</span>
        <div className="grid grid-cols-2 gap-1 rounded-lg border border-[#1e4060] bg-[#061827] p-1">
          <button
            ...
            className={`flex items-center justify-center rounded-md px-2.5 py-1.5 text-[14px] font-medium transition-all ${current.timingMode === "now" ? "bg-blue-600/35 text-white border border-blue-400" : "text-[#7eb8e0] hover:text-white"}`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center rounded-md px-2.5 py-1.5 text-[14px] font-medium transition-all ${current.timingMode === "custom" ? "bg-blue-600/35 text-white border border-blue-400" : "text-[#7eb8e0] hover:text-white"}`}
          >
            Custom
          </button>
        </div>
        {current.timingMode === "custom" ? (
          <div className="mt-1.5 flex h-9 w-full items-center rounded-lg border border-[#1e4060] bg-[#061827] px-3 focus-within:border-[#4f8ef7]">
            <input
              ...
              placeholder="05:06"
              className="h-full min-w-0 flex-1 bg-transparent text-[14px] font-medium text-[#c8d8ea] outline-none placeholder:text-[#31516b]"
            />
            <span className="ml-2 shrink-0 text-[14px] font-medium text-[#7eb8e0]">hrs</span>
          </div>
        ) : (
          <div className="mt-1.5 flex h-8 items-center gap-2 rounded-lg border border-[#1e4060] bg-[#061827] px-3 text-[14px] font-medium text-[#7eb8e0]">
  ...
  const renderMovementFormCard = (operation) => {
    ...
    const labelClass = "mb-1.5 block text-[14px] font-medium uppercase tracking-[0.12em] text-[#58a6ff]";
    const inputClass = "h-9 w-full rounded-lg border border-[#1e4060] bg-[#061827] px-3 text-[14px] font-medium text-white outline-none placeholder:text-[#31516b] focus:border-[#4f8ef7]";
          ...
          <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-12 lg:items-end">
            <label className="col-span-1 lg:col-span-2">
              ...
              <div className="flex h-9 items-center gap-2 rounded-lg border border-[#1e4060] bg-[#061827] px-3 focus-within:border-[#4f8ef7]">
                <span className="text-[14px] font-medium text-[#4f8ef7]">T</span>
                <input
                  value={current.trainId}
                  onChange={(e) => updateMovementForm(operation, "trainId", e.target.value.replace(/\D/g, ""))}
                  placeholder="e.g. 25"
                  className="h-full min-w-0 flex-1 bg-transparent text-[14px] font-medium text-white outline-none placeholder:text-[#31516b]"
                />
              </div>
            </label>
                  ...
                  {selectedRoads.map((road) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border px-2 py-1.5 text-[14px] font-medium transition-all ${active ? "border-blue-400 bg-blue-600/30 text-white" : "border-[#1e4060] bg-[#061827] text-[#7eb8e0] hover:border-[#4f8ef7] hover:text-white"}`}
                      >
                        {road}
                      </button>
                    );
            ...
            <label className="col-span-2 lg:col-span-5">
              ...
              <textarea
                ...
                className="min-h-[42px] w-full resize-none rounded-lg border border-[#1e4060] bg-[#061827] px-3 py-2 text-[14px] font-medium text-white outline-none placeholder:text-[#31516b] focus:border-[#4f8ef7]"
              />
            </label>

            <div className="col-span-2 self-stretch rounded-lg border border-[#1e4060] bg-[#061827] px-3 py-2 lg:col-span-7">
              <p className="mb-1 text-[14px] font-medium uppercase tracking-[0.12em] text-[#4a8ab5]">Preview</p>
              <p className="font-mono text-[14px] font-medium leading-snug text-[#c8d8ea]">
            ...
            <button
              ...
              className="col-span-2 flex h-9 w-full items-center justify-center gap-1.5 rounded-lg border text-[14px] font-medium text-white shadow-[0_0_16px_rgba(59,130,246,0.18),inset_0_1px_0_rgba(255,255,255,0.08)] transition-all hover:scale-[1.01] lg:col-span-12"
