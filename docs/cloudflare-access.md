# Depot Controller-only Cloudflare Access

The application must be protected by Cloudflare Access before it is used with operational data. The Pages middleware also fails closed unless it receives a valid Access JWT and the verified email is present in the private 10-member Depot Controller allowlist.
...
## 2. Upload the exact 10-member Depot Controller email list
...
2. Create an `EMAIL` list and upload a private CSV using this format:
   ...
   ```csv
   value,description
   member@example.com,Depot Controller
   ```

3. Confirm the list contains exactly 10 unique email addresses.
...
- **Include:** Emails in list = the 10-member Depot Controller email list
...
| `OCC_ALLOWED_EMAILS` | Encrypted secret | The exact 10 Depot Controller addresses, separated by commas or new lines |

The required count of 10 is fixed in reviewed code and cannot be expanded with a dashboard variable. The middleware returns `503` when any required setting is missing or the private allowlist does not contain exactly 10 unique addresses. It validates the `Cf-Access-Jwt-Assertion` signature against Cloudflare's rotating JWKS, plus the issuer, audience, expiry, and email membership.
...
4. Adding or removing an address causes the middleware to fail closed when the unique count is not 10. Replacing one address while keeping 10 entries immediately changes who is authorized after the secret is redeployed.
